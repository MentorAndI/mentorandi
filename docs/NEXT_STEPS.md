...
3. Test real provider connectivity, usage diagnostics, model routing, cost controls and `npm run eval:mentor` before alpha; production alpha must use OpenAI or Anthropic, OpenAI quota/billing must work, and Claude requires `ANTHROPIC_API_KEY` plus `ANTHROPIC_MODEL`.
