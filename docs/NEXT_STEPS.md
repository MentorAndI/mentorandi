# MentorAndI Next Steps

## Immediate Next Features

1. Improve goal dedupe and goal updates.
2. Add reflection UI or internal dev visibility.
3. Verify OpenAI and Anthropic billing/quota and test real model responses in real provider mode.
4. Add production auth hardening.
5. Add account/privacy controls.
6. Deploy alpha.
7. Improve visual design.
8. Add basic logging/error handling.
9. Expand provider diagnostics and logging after real provider testing.
10. Add a pre-deployment checklist for production environment variables and auth configuration.
